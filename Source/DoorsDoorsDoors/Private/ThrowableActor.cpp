...
void AThrowableActor::NotifyHit(UPrimitiveComponent* MyComp, AActor* Other, UPrimitiveComponent* OtherComp, bool bSelfMoved, FVector HitLocation, FVector HitNormal, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	if (State == EState::Launch)
	{
		...
		{
			if (ADoorsDoorsDoorsPlayerCharacter* DDDPC = Cast<ADoorsDoorsDoorsPlayerCharacter>(Other))
			{
				DDDPC->NotifyHitByThrowable(this);
			}
		}
	}
	...
	{
		if (ADoorsDoorsDoorsPlayerCharacter* DDDPC = Cast<ADoorsDoorsDoorsPlayerCharacter>(PullActor))
		{
			if (Other == PullActor)
			{
				AttachToComponent(DDDPC->GetMesh(), FAttachmentTransformRules::SnapToTargetNotIncludingScale, TEXT("hand_r"));
				SetOwner(DDDPC);
				ProjectileMovementComponent->Deactivate();
				State = EState::Attached;
				//set character state to attached
				DDDPC->OnThrowableAttached(this);
			}
			else
			{
				DDDPC->ResetThrowableObject();
